...
int ses_init(int capacity) {
  ...
  sessions.rooms = hashmap_init(capacity, hashmap_strkey_cmp, NULL,
...
int ses_room_jion(const char *room_name, struct client *c) {
  if (!room_name || !c)
    return -1;

  hashmap *one_room = NULL;
  int ret = hashmap_get(sessions.rooms, (void *) room_name, (void **) &one_room);
  if (ret == HASHMAP_OK) {
    // room exist
...
int ses_room_leave(const char *room_name, struct client *c) {
  if (!room_name || !c)
    return -1;
